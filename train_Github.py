...
def run(data,model,optimizer,cfg):
    ...
    test_acc,hs = test(data,model)
    return test_acc,early_stopping.epoch,hs
...
def main(cfg):
    ...
    for i in tqdm(range(cfg['run'])):
        ...
        test_acc,epoch,hs = run(data,model,optimizer,cfg)
        for j in range(len(hs)):
            artifacts[f"{cfg['dataset']}_{cfg['att_type']}_h_{i}_test_L{j}.npy"] = hs[j]
